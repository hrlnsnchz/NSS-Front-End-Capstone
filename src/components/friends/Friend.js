...
export const FriendCard = ({friend, userObject, currentUser, reversedRoleCurrentUser}) => {
  console.log('reversedRoleCurrentUser: ', reversedRoleCurrentUser);
  ...
  <section className="friend">
        ...
        <Link to={`/profile/${userObject?.id}`}>{friend.currentUserId === currentUser.id? userObject.name + " " + "is your Friend  " : ""}</Link>
        <Link to={`/profile/${reversedRoleCurrentUser?.id}`}>{friend.userId === currentUser.id? reversedRoleCurrentUser.name + " " + "is your Friend  ": ""}</Link>
